...
- The Start Walkthrough (chapters & steps)
...
- `returnsRegime.ts` — Hardy-style 2-regime switching engine, fit/simulated on simple annual returns (bull/bear hidden Markov chain).
...
The senior-bonus **phaseout** is the subtle part. It reduces the *aggregate* deduction (the combined `6,000 × num65Plus`) by `SENIOR_BONUS_PHASEOUT_RATE = 6%` of **AGI** over the threshold, **once** — not each filer's $6,000 independently. (OBBBA §70103's MAGI adds back only §911/931/933 foreign exclusions — municipal-bond interest is NOT added back, so the engine phases out on AGI:)

```
gross = 6,000 × num65Plus
return max(0, gross − max(0, agi − phaseoutStart) × 0.06)
```
...
`NIIT_RATE = 0.038`. The base is net investment income = `qualifiedDividends + longTermGains + taxableInterest + ordinaryDividends`. Tax is 3.8% of the **lesser** of NII or MAGI-over-the-threshold — where §1411 MAGI is AGI plus only §911 foreign-earned-income exclusions, so tax-exempt (muni) interest is **not** added back and the engine uses AGI:

```
niit = 0.038 × max(0, min(netInvestmentIncome, agi − niitThreshold))
```
...
8. **Pay the IRMAA cash cost.** The Medicare IRMAA surcharge (`plan.tax.irmaa.householdAnnual`) is **not** an income tax and is **not** inside `plan.tax.totalTax` or `netCash`. It is a real out-of-pocket cash outflow paid this year so those dollars leave the compounding base. A forced-RMD surplus (`leftover = plan.netCash - plan.spendingTarget`) covers it first; `reinvestAmt = max(0, leftover - irmaaCost)` is reinvested into the brokerage as new full-basis money (`reinvestSurplus` — strictly a *brokerage* account, opening one if none exists, never non-growing cash), while any uncovered premium (`premiumFromSavings`) is drawn from savings cash-first.
...
11. **Settle the year's dividend & interest income, by mode.** In **spend** mode, `distributeFromBrokerage` removes this year's `brokerageDividendsAnnual + ordinaryDividendsAnnual + taxExemptInterestAnnual` from the brokerage balance immediately after growth (balance only — dividends aren't return of capital, so basis is unchanged and the unrealized gain correctly shrinks): those dollars were received as cash funding spending, and leaving them to *also* compound would double-count versus a tax-free Roth. In the default **reinvest** mode two credits keep the books honest: the year's `taxableInterestAnnual` is credited to the cash accounts (cash is non-growing in `growAll`, so unlike brokerage dividends — which ride inside total-return growth — taxed interest would otherwise simply vanish), and the retained brokerage payouts step up the brokerage **cost basis** (a taxed-and-retained dividend is after-tax new money, exactly like a real reinvested distribution — without this it would be taxed again as unrealized gain on a later sale). This mode split is the structural reason a taxable account lags a Roth in the model.
...
- **Regime-switching — `runRegimeSwitching`** (default 1,000 runs, seed 24680). A Hardy RSLN-2 two-state hidden Markov model (the actuarial-reserving standard), calibrated **offline by EM** to S&P 500 annual returns 1928–2024 (statsmodels `MarkovRegression`; `research/regimes.py`, stored in `lib/calibrated/regimes.json`). A calm bull regime is punctuated by a negative-mean bear regime that **clusters** (P(bear|bear) ≈ 0.36 vs an unconditional ~0.12). The bear's danger is its negative mean and persistence, not higher single-year vol (bull ~15.9%, bear ~13.3%). To make it a clean apples-to-apples check, the equity regime means are shifted so the stationary blended mean equals the CMA equity mean, and dispersion is scaled (`mixVar`) so the stationary mixture vol equals the CMA equity vol; bonds, cash and inflation use the same 4×4 Cholesky correlation and AR(1) inflation as the main engine (as plain Gaussians on simple returns — without the main engine's lognormal/Student-t dressing, immaterial at bond/cash vols). The result surfaces `regimeInfo` (retargeted bull/bear means and bull weight) so the UI shows the true simulated assumptions. (The bear regime is identified from only ~11 of 97 years, so its parameters carry wide error bars.)
...
The guided walkthrough is the app's primary on-ramp, defined entirely in `components/GuidedPlan.tsx`. Steps are pushed into a `steps: Step[]` array (`type Step = { key; chapter; eyebrow; render }`) and stable-sorted into the fixed **chapter order** — `opening → you → money → income → goal → spending → markets → review` — so the flow always reads as one advisor-style journey: who you are → your money → your income → your goal → spending → assumptions & the rollover → review. A desktop chapter rail / mobile chapter header shows where you are; conditional steps never make the counts lie (both are derived from the steps actually present). The walkthrough is also the **complete questionnaire**: every consequential input in the app has its home on one of these steps (the other tabs show settings read-only with an `Adjust →` deep-link back to the owning step), so nothing important can be discovered later in a settings panel.
...
| Chapter | `key` | What it decides / shows |
|---|-------|-------------------------|
| opening | `start` | The one-time fork: **own numbers vs. the $5M example**. |
| you | `aboutyou` | **Birth year(s)** (you + optional spouse) and the **year you plan to start retirement**. |
| you | `longevity` | **Plan-to age**, optional **sex** (longevity context only) + the survivor (widow's-penalty) model. |
| money | `bucket-pretax/roth/brokerage/cash` | Simple balances-first entry: one total per tax bucket (own mode with un-itemized accounts only; brokerage also asks embedded gains). |
| money | `accounts` | Your portfolio, totaled and broken out **by tax bucket** (read-only overview; itemized editing lives on /accounts). |
| income | `ssclaim` | **Social Security claim age** + full (FRA) benefit, with the optimizer's suggestion. |
| income | `otherincome` | **Pension/annuity** and the **dividends & interest** the taxable accounts pay (qualified / ordinary / taxable interest / muni) — read-only when per-holding dividend data drives them. |
| income | `dividends` | Reinvest the payout (default) vs. take it as cash. |
| goal | `goal` | maxCapital / lowestTax / lowestRate (+ Advanced expander: withdrawal method & bracket-fill target). |
| goal | `mostmoney` | The win-rate / median / mean tie-break (**maxCapital only**). |
| spending | `spend` | Spending level + spending-strategy toggle + live tax/IRMAA preview. |
| markets | `markets` | Return + inflation assumptions. |
| markets | `rollconfirm` | Confirm this year's Roth rollover (+ Advanced expander: conversion window & heir tax rate). |
| markets | `fund` | What pays for it + withdrawal order (claim ages shown with a jump-link back to `ssclaim` — no duplicate editor). |
| review | `roll` | The lifetime tax-smoothing comparison (desktop-depth). |
| review | `ahead` | The multi-year action plan. |
| review | `breakdown` | The choices recap (tap any line to change it) + all supporting detail in collapsed sections — the mobile home for desktop-only depth. |
| review | `done` | The Monte-Carlo confidence verdict. |

**`start` — own vs. example.** The fork that the whole walkthrough runs on. Two buttons: "Use my own numbers" (`setMode("own")`) and "Explore the $5M example" (`setMode("demo")`). There is deliberately no mid-flow toggle — to switch, you come back here (Back from step 2, or "Start over" at the end) and re-pick. `needsOwnSetup = mode === "own" && household.accounts.length === 0` is computed here and gates later steps.

**`accounts` — portfolio by tax bucket.** Shows the total across all accounts (animated) and `AccountOverview`, which lays the holdings out *by tax treatment*. In `demo` mode it banners the sample `{moneyCompact(total)}` household and offers "🎲 New example"; in `own` mode it offers "Edit my accounts." If `needsOwnSetup`, this step instead shows an "Add my accounts" call to action linking to `/accounts` and the rest of the flow's body is suppressed.
...
**`longevity` — plan-to age, optional sex, survivor model.** An optional per-person sex picker (Female/Male/Skip→blended) feeds ONLY the Gompertz longevity context on Forecast — never the tax math — and lives here so every input has a questionnaire home.  *(Skipped when `needsOwnSetup`.)* Picks `endAge` from {90, 95, 100, 105} (95 labeled "typical"). The horizon drives every downstream number. If there's a spouse, a `survivorModel` toggle (**on by default**) models the survivor filing as single (narrower brackets) and keeping the larger Social Security — the widow's penalty — with a `firstDeathAge` picker {80, 85, 88, 90, 92}.

**`ssclaim` — claim age with the optimizer's suggestion.** *(Skipped when `needsOwnSetup`; only shown when `hasSS`, i.e. someone has a positive `socialSecurityAnnual`.)* A per-person `ClaimPicker` offers {62, FRA, 70} (FRA from `fullRetirementAge(birthYear)`), each button showing the `adjustedAnnualBenefit(...)` for that age. When `rec.claimAdvice.delayWho` exists, a Callout surfaces the optimizer's suggested pair (`adv.self / adv.spouse`); for `maxCapital` with `adv.lift > 1000` it quantifies the projected lifetime gain (`moneyCompact(adv.lift)` more than the current ages). It is explicitly framed as a suggestion — the user still sets what they prefer.

**`otherincome` — pension + investment income.** The guided home for `pensionAnnual` and the four household investment-income fields (`brokerageDividendsAnnual` qualified, `ordinaryDividendsAnnual`, `taxableInterestAnnual`, `taxExemptInterestAnnual`), with 1099-oriented helper copy. When per-holding dividend data exists (`dividendBreakdown(...).hasData`), the dividend figures are computed from holdings and shown read-only with a link to /accounts. In demo mode the fields are read-only (the example's numbers). A desktop-only explainer covers why each type is asked separately (preferential vs ordinary rates; muni's IRMAA/SS-taxability role; Illinois' retirement-income exemption).

**`dividends` — reinvest vs spend.** Only when the taxable accounts pay > $200/yr: choose `dividendMode` — reinvest (default; stays invested, tax still owed) or take as cash (covers spending, withdraw less).

**`goal` — maxCapital / lowestTax / lowestRate.** Maps over `GOALS = ["maxCapital", "lowestTax", "lowestRate"]` (`GuidedPlan.tsx` L39). Tapping a goal calls `applyGoal(g)`, which recommends at the stable reference spending and writes the goal's recommended config with `planCustomized: false`. Each button shows a one-line `planGist(recAll[g])`, so differences between goals are visible. A footer note checks `goalsAgree` (via `configMatches`): if all three goals converge on the *same* plan it says so (and whether that plan includes Roth conversions); if they diverge it points the user at the per-button summaries. An **Advanced expander** on this step is the single editable home for the withdrawal *method* (`strategy`: smart / conventional / proportional) and the bracket-fill target (`bracketTarget`); both set `planCustomized: true`. The same controls on /plan are read-only with an `Adjust →` link back here.

**`mostmoney` — the win-rate / median / mean choice.** *(Only when `settings.goal === "maxCapital" && finalists.length >= 2`.)* Runs the top finalists through the *same* hundreds of simulated markets and ranks them by the chosen `mostMoneyMetric`: `winRate` ("Wins most often" — default, the planner's "most likely to leave you the most"), `median` ("Highest typical" — steadiest), or `mean` ("Highest average" — chases upside). The top-ranked plan becomes the active plan immediately (changing the metric writes `mostMoneyMetric` with `planCustomized: false` so it re-picks and auto-applies). While the worker runs (`!mmFresh`) it shows a "Running your plans through the markets…" placeholder; once fresh it renders each finalist with Wins% / Typical / Average and a "Recommended" pill on the winner.

**`spend` — spending + inflation-growth toggle + tax/IRMAA preview.** The core spending screen. A slider over a `sweep` exposes a comfortable ceiling (green), a sustainable max (amber), and a recommended ~4%-pace "quick amount"; a marker rail places those ceilings and every IRMAA cliff (`impact.irmaaCliffs`, red) at the spend level where each bites. The headline readout shows annual after-tax spend; a live tax/IRMAA preview card (fed by the live slider MAGI, not the committed plan) shows the resulting marginal rate, Medicare (IRMAA) tier, and ending-savings value, including whether the IRMAA tier is `irmaaPinned` by other income. A **`spendingStrategy` toggle** chooses "Grow with inflation" (`constant` — the default, keeps real spending steady, showing the grown value at `endAge`) vs. "Stay flat" (`flatNominal`).

**`markets` — return + inflation assumptions.** Sets the *middle* of every projection (the forecast then stress-tests hundreds of paths around it). Return after fees from {4% Cautious, 5% Moderate, 6% Optimistic}; inflation from {2%, 2.5%, 3%, 3.5%}.

**`rollconfirm` — confirm this year's Roth rollover.** *(Only when `pretaxShare > 0.2`.)* Builds the picture in order: it adds the rollover's taxable income *on top of* the spending just chosen and re-shows the Medicare (IRMAA) tier before vs. after (`irmaaCliffInfo` on `planNoConv` vs `planWithRoll`). Two branches:
...
**`fund` — what pays for it + withdrawal order.** Shows the funding split for the current year: guaranteed income first (Social Security, pension, dividends & interest, itemized so "guaranteed" isn't a black box), then the savings draw to fill the gap, with a withdrawal-rate read color-coded against the ~4.5%/6% thresholds. The savings draw is itemized in spend order — RMD first (forced, ordinary income), then voluntary pre-tax (low-bracket fill), then **taxable cash-first then brokerage**, then Roth last (tax-free, used last so it compounds longest). It also ranks the three withdrawal *orders* — `conventional` (brokerage & cash first), `smart` (pre-tax first / fill low brackets), `proportional` (a little from everything) — by after-tax keep and marks the chosen one with its edge over the runner-up.

**`roll` — the lifetime smoothing comparison.** *(Only when `pretaxShare > 0.2`.)* Compares three lifetime strategies side by side from `compare`: "Do nothing extra" (`compare.none` — RMDs arrive in big high-bracket chunks), "Smooth (recommended)" (`compare.smooth` — small rollovers staying in low brackets), and "Convert aggressively" (`compare.aggressive` — fill the bracket every year). It quantifies how much of the best-case gain smoothing captures (`captured`), the smoothing gain vs. doing nothing, and the aggressive plan's remaining edge — with every numeric claim dynamic and phrased to stay true whether aggressive helps, ties, or loses. The long per-scenario explanations and the low-tax / zero-federal-bracket explainers are `DesktopOnly`.

**`ahead` — the multi-year action plan.** "Your next few years, at a glance": maps `lookAhead` into tappable `AheadYearRow`s, each expandable to everything that year involves (the concrete year-by-year actions implied by the chosen plan).
...
- `bucket-*`: only own mode while every account id is a `guided-*` bucket (itemizing on /accounts converts them to a read-only view here).
- `ssclaim`: requires `hasSS` (someone has a positive benefit); suppressed while `needsOwnSetup`.
- `dividends`: only when the taxable accounts pay > $200/yr.
...
- `start`, `aboutyou`, `longevity`, `accounts`, `otherincome`, `goal`, `spend`, `markets`, `fund`, `ahead`, `breakdown`, `done` are always present.

**Key source locations** (`components/GuidedPlan.tsx`): chapter model (`ChapterId`, `CHAPTER_ORDER`, `CHAPTERS`, `STEP_CHAPTER`) at the top of the file; each step is a `steps.push({ key, chapter, eyebrow, render })` call — search the file for `key: "<step>"`; `defaultRetirementYear` in `lib/accounts.ts`; demo-vs-own exploratory layering via `EXPLORATORY_KEYS` in `components/HouseholdProvider.tsx`; `DesktopOnly`/`Collapsible`/`AdjustLink` in `components/ui.tsx`; `GOALS`/`GoalId` from `lib/goals.ts`.
