...
const TOPICS: Topic[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "Market risk is handled in two layers: the year-by-year plan uses one steady return for clarity, and the separate confidence check runs hundreds of randomized-return simulations (Monte Carlo) to show sequence-of-returns risk and the range of outcomes. A few things are deliberate simplifications — treat them as judgment calls: state tax is Illinois only; pre-65 ACA premium subsidies aren't modeled; any after-tax (nondeductible) IRA basis is ignored, so every conversion is treated as fully taxable (the IRS pro-rata rule); and inherited pre-tax is valued as drained over an heir's 10-year window at an assumed heir bracket (24% by default, adjustable on the Forecast tab). You can also switch the whole forecast to today's dollars there. Everything here is an educational estimate — confirm with a tax professional before acting.",
    ],
