...
const METHODS: Method[] = [
  ...
  },
  {
    ...
    how: `Your money's “average” return can be told two ways, and this app uses both on purpose. The arithmetic average is the simple year-to-year average (the app's “expected” return — for example, the 7.94% it assumes for U.S. stocks); the geometric average is the rate your balance actually compounds at after good and bad years partly cancel out, and it's always lower. The app gets the compound rate by subtracting “volatility drag” — roughly half the variance — from the arithmetic average. It uses the higher arithmetic mean in the random year-by-year Monte-Carlo simulation (the correct single-year expectation) but compounds the lower geometric rate as the steady “Expected” straight-line scenario, so it never overstates your likely ending balance. (The separate “History repeated” option instead compounds what your exact stock/bond/cash mix actually averaged over 1928–2024 — higher, because it assumes the past repeats.)`,
