...
const TOPICS: Topic[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "Because no single return model is the whole truth, the Forecast tab also lets you re-run your exact plan through two independent SECOND OPINIONS. The historical BLOCK-BOOTSTRAP draws multi-year blocks of actual 1928–2024 market history (like cFIREsim), capturing real crashes and the way stocks, bonds, and inflation truly moved together. The REGIME-SWITCHING engine (Hardy's RSLN-2, the model actuaries use for capital reserving) alternates between a calm bull market and a turbulent bear market that persists, so bad years cluster into multi-year drawdowns. All three engines are detrended to the same long-run averages, so when they land in the same neighborhood — they typically do, within a few points — your result is robust to how returns are modeled, not an artifact of one model's shape. (We validate this offline too: five engines run through a common withdrawal model agree on the success rate to within ~5 points.)",
